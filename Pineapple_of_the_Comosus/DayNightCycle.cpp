...
{   
    ...
    if (period == 3) SunReset();
        ...
        if (elapsedTime < 1.0f) {
            m_Sun->transform->pos.worldPosition.y = (1-elapsedTime) * sphereLocation[period] + elapsedTime * sphereLocation[period + 1];
        }
...
{
	m_Sun->transform->pos.worldPosition.y = sphereLocation[0];
}
