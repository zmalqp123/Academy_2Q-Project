...
SpriteUIAnimation::SpriteUIAnimation()
{
}
...
void SpriteUIAnimation::LoadAnimationAsset(const std::wstring strFilePath)
{
...
void SpriteUIAnimation::LoadTexture(const std::wstring strFilePath)
{
	m_strTextureFilePath = strFilePath;

	ResourceManager::GetInstance().CreateTextureFromFile(m_strTextureFilePath, &m_pTexture);
}
...
void SpriteUIAnimation::Render(ID2D1HwndRenderTarget* pRenderTarget, D2D1_MATRIX_3X2_F cameraMat)
{
...
void SpriteUIAnimation::Render(D2D1_MATRIX_3X2_F cameraMat)
{
	if (gameObject->isActive == false || (gameObject->transform->m_pParentScene != nullptr && gameObject->transform->m_pParentScene->gameObject->isActive == false))
		return;
	if (m_pAnimationInfo == nullptr)
...
void SpriteUIAnimation::SetAnimation(int index, bool mirror, bool continueCurrentFrame)
{
...
AABB SpriteUIAnimation::GetBound()
{
	AABB ab;
...
int SpriteUIAnimation::GetMaxIndex()
{
	return m_pAnimationInfo->Frames.size();
}

bool SpriteUIAnimation::IsLastFrame()
{
